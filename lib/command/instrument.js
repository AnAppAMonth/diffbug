...
var path, mkdirp, async, fs, filesFor, nopt, Instrumenter,
    inputError, flowControl, verbose;
...
Command.mix(InstrumentCommand, {
    ...
    usage: function () {
        console.error('\nUsage: ' + this.toolName() + ' ' + this.type() + ' <options> <file-or-directory>\n\nOptions are:\n\n' +
            [
                ...
                formatOption('--variable <global-trace-variable-name>', 'change the variable name of the global trace variable from the ' +
                    'default value of `__trace__` to something else'),
                formatOption('--embed-source', 'embed source code into the trace object, defaults to false'),
    ...
    run: function (args, callback) {
        ...
        var config = {
            ...
            stream,
            includes,
            instrumenter = new Instrumenter({ traceVariable: opts.variable });
