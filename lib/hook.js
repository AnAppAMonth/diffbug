 ...
 * function that performs the actual transform. Instrumenting code for coverage is one
 * specific example of useful hooking.
 *
 * Note that both the `matcher` and `transformer` must execute synchronously.
 *
 * For the common case of matching filesystem paths based on inclusion/ exclusion patterns,
 * use the `matcherFor` function in the API to get a matcher.
 *
 * It is up to the transformer to perform processing with side-effects, such as caching,
 * storing the original source code to disk in case of dynamically generated scripts etc.
 * The `Store` class can help you with this.
 *
 * Usage
 * -----
 *
 *      var hook = require('diffbug').hook,
