...
(function (isNode) {
    ...
    function generateTrackerVar(filename, omitSuffix) {
        ...
        if (crypto !== null) {
            ...
            window.__trc_seq = window.__trc_seq || 0;
            window.__trc_seq += 1;
            suffix = window.__trc_seq;
        }
        return '__trc_' + (omitSuffix ? '' : suffix);
    }
    ...
    /**
     * Mechanism to instrument code for tracing. It uses the `esprima` and
     ...
     * ---------------
     *
     *      var instrumenter = new require('diffbug').Instrumenter(),
     ...
     * @param {String} [options.traceVariable] the global variable name to use for
     *      tracking the execution trace. Defaults to `__trace__`
     * @param {Boolean} [options.embedSource] whether to embed the source code of every
     *      file as an array in the file trace object for that file. Defaults to `false`
     ...
     *      an anonymous function before executing it. By default, code is wrapped in
     ...
     *      setting this option is a pretty-print of the trace variable. Defaults to `false`
    ...
    function Instrumenter(options) {
        ...
        this.walker = new Walker({
            ExpressionStatement: this.traceStatement,
            BreakStatement: this.traceStatement,
            ContinueStatement: this.traceStatement,
            DebuggerStatement: this.traceStatement,
            ReturnStatement: this.traceStatement,
            ThrowStatement: this.traceStatement,
            TryStatement: this.traceStatement,
            VariableDeclaration: this.traceStatement,
            IfStatement: [ this.ifBlockConverter, this.ifBranchInjector, this.traceStatement ],
            ForStatement: [ this.skipInit, this.loopBlockConverter, this.traceStatement ],
            ForInStatement: [ this.skipLeft, this.loopBlockConverter, this.traceStatement ],
            WhileStatement: [ this.loopBlockConverter, this.traceStatement ],
            DoWhileStatement: [ this.loopBlockConverter, this.traceStatement ],
            SwitchStatement: [ this.switchBranchInjector, this.traceStatement ],
            WithStatement: this.traceStatement,
            FunctionDeclaration: [ this.traceFunction, this.traceStatement ],
            FunctionExpression: this.traceFunction,
            LabeledStatement: this.traceStatement,
    ...
    Instrumenter.prototype = {
         ...
         *  used for embedding the source into the trace object
         */
        ...
        fixColumnPositions: function (traceState) {
        ...
        getPreamble: function (sourceCode) {
            var varName = this.opts.traceVariable || '__trace__',
                file = this.traceState.path.replace(/\\/g, '\\\\'),
                ...
                code;
            if (!this.opts.noAutoWrap) {
                this.fixColumnPositions(this.traceState);
            }
            if (this.opts.embedSource) {
                this.traceState.code = sourceCode.split(/\n/);
            }
            traceState = this.opts.debug ? JSON.stringify(this.traceState, undefined, 4) : JSON.stringify(this.traceState);
            code = [
                ...
                .replace(/%OBJECT%/g, replacer(traceState));
            return code;
        },
        ...
        statementName: function (location, initValue) {
            ...
            this.traceState.statementMap[sName] = location;
            this.traceState.s[sName] = initValue;
            return sName;
        },
        ...
        traceStatement: function (node, walker) {
            var sName,
                incrStatementCount,
                grandParent;
        ...
        functionName: function (node, line, location) {
            ...
            this.traceState.fnMap[id] = { name: name, line: line, loc: location };
            this.traceState.f[id] = 0;
            return id;
        },

        traceFunction: function (node, walker) {
        ...
        branchName: function (type, startLine, pathLocations) {
            ...
            this.traceState.b[bName] = paths;
            this.traceState.branchMap[bName] = { line: startLine, type: type, locations: locations };
            return bName;
        },
